...
async def add_new_transaction(request: Request, body: WebhookBody):
    ...
    async with session.begin():
        new_transaction = Transaction(
            id=body.transaction_id, bill_id=body.bill_id, amount=body.amount
        )
        session.add(new_transaction)
        await session.execute(
            update(Bill)
            .where(Bill.id == body.bill_id)
            .values(balance=Bill.balance + body.amount)
        )
...
def check_signature(body: WebhookBody):
    ...
    new_signature.update(
        f"{PRIVATE_KEY}:{body.transaction_id}:{body.user_id}:{body.bill_id}:{body.amount}".encode()
    )
...
async def update_user_balance(request: Request, bill_id: int, price: float):
    ...
    async with session.begin():
        await session.execute(
            update(Bill).where(Bill.id == bill_id).values(balance=Bill.balance - price)
        )
...
async def buy_goods(request: Request, goods_id: int):
    ...
    bill_balance: float = [
        bill_object.balance
        for bill_object in current_user.bill
        if bill_object.id == bill_id
    ][0]
